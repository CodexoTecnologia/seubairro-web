...
const ERROR_MESSAGES: Record<string, string> = {
  AddressRequired: 'Informe seu endereço.',
...
export const ClientForm: React.FC = () => {
  ...
  const goNext = async () => {
    const ok = await trigger(CLIENT_SIGNUP_STEP_FIELDS[step])
    if (!ok) return
    setStep(2)
  }
      ...
      {step === 2 && (
        <>
          <p className="text-xs text-[var(--color-muted)]">
            Informe seu <strong>endereço</strong>. Usamos ele para mostrar produtos e serviços perto
            de você.
          </p>
          <Input
            label="CEP"
